...
public class ZombieSpawnManager {
  ...
  public void spawnZombies() {
    ...
    AreaConfig config = VillageDefender.getArea(arena.getId());
    if (config == null) {
      oriSpawnZombies();
      return;
    }
    WaveConfig wave = config.getWave(arena.getWave());
    if (wave == null) {
      oriSpawnZombies();
      return;
    }
    if (!config.isMustSpawn()) {
      wave.getMustSpawn().forEach((k, v) -> {
        for (int i = 0; i < v; i++) {        
          spawnMMZombies(k);
        }
      });
    ...
    ArrayList<String> mobList = wave.getRandomSpawn();
    if (mobList.isEmpty()) {
      oriSpawnZombies();
      return;
    }
    String name = null;
    for (int j = 0; j < 5; j++) {
      name = mobList.get(random.nextInt(mobList.size()));
      if (wave.checkSpawn(name)) {
        break;
      }
    }
    spawnMMZombies(name);
